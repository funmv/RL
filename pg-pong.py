...
수식으로 쓰보면, $ra = ra*g+r_t$ -> $(ra*g+r_t)*g+r_{t+1}$ 형태의 재귀 폼이다. 두 단계만 전개해보면
$((ra*g+r_t)g+r_{t+1})g+r_{t+2} = (ra*g^2+r_t*g+r_{t+1})g+r_{t+2} = ra*g^3+r_t*g^2+r_{t+1}*g+r_{t+2}$이고 초기 action에서 멀어지면 약하게 가중되는 수식이다
"""  
